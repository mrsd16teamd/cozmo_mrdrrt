...
class HeadLiftJoy(object):
    ...
    def __init__(self):
        ...
        """
        # params
        self._head_axes = rospy.get_param('~head_axes', 5)
        self._lift_axes = rospy.get_param('~lift_axes', 5)
        self._head_button = rospy.get_param('~head_button', 0)
        self._lift_button = rospy.get_param('~lift_button', 1)
        ...
        self._angle_deg = DEFAULT_HEAD_ANGLE
        self._lift_mm = MIN_LIFT_HEIGHT
    ...
    def _joy_cb(self, msg):
        ...
        if msg.buttons[self._head_button] == 1:
            ...
            angle_deg += (msg.axes[self._head_axes])*2.0
            angle_deg = max(angle_deg, MIN_HEAD_ANGLE)
            angle_deg = min(angle_deg, MAX_HEAD_ANGLE)
        ...
        if msg.buttons[self._lift_button] == 1:
            ...
            lift_mm += (msg.axes[self._lift_axes])*2.0
            lift_mm = max(lift_mm, MIN_LIFT_HEIGHT)
            lift_mm = min(lift_mm, MAX_LIFT_HEIGHT)
            self._lift_mm = lift_mm
            lift_mm = (lift_mm-MIN_LIFT_HEIGHT)/SUM_LIFT_HEIGHT
            ...
            self._lift_pub.publish(data=lift_mm)
